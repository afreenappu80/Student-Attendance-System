      ...
      <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {notifications.map(n => (
              <div key={n.id} className={`p-6 rounded-2xl border ${!n.is_read ? 'border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800' : 'border-gray-100 bg-white dark:bg-gray-800 dark:border-gray-700'} flex items-start justify-between group transition-colors`}>
                ...
                <button onClick={() => handleDelete(n.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition opacity-0 group-hover:opacity-100">
                  <FiTrash2 />
                </button>
              </div>
            ))}
